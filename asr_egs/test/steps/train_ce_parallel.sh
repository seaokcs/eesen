...
train_tool=train-ce-parallel  # the command for training; by default, we use the
...
for iter in $(seq $start_epoch_num $max_iters); do
    ...
    tracc=$(cat $dir/log/tr.iter${iter}.log | grep "FRAME_ACCURACY" | tail -n 1 | awk '{ acc=$3; gsub("%","",acc); print acc; }')
    ...
    cvacc=$(cat $dir/log/cv.iter${iter}.log | grep "FRAME_ACCURACY" | tail -n 1 | awk '{ acc=$3; gsub("%","",acc); print acc; }')
